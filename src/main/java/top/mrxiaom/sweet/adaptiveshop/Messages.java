...
public enum Messages implements IHolderAccessor {
    ...
    help_op("", "&d&lSweetAdaptiveShop &c&l帮助命令&r&f &7<&e必选参数&7> &7<&e选项1&7/&e选项2&7> [&e可选参数&7]",
            ...
            "&f/sashop dynamic &7<&ebuy&7/&esell&7> <&e商品ID&7> <&eset&7/&eplus&7/&eminus&7> <&e数值&7> &7[&e玩家&7]",
            "  &8-- &7设置指定商品的动态值，玩家仅在商品要求每个玩家独立动态值时输入",
            "&f/sashop test &7<&eorder&7/&ebuy&7/&esell&7> <&e玩家&7>",
            "  &8-- &7查看玩家当前的订单/商品列表，以及到期时间",
    ...
    gui__sell__success_message("&a你花费 &e%money%&r &a，购买了 &e%amount%&a 个 &e%item%&r&a!"),
    ...
    dynamic__number_invalid("&e请输入正确的数值"),
    dynamic__shop_type_invalid("&e请输入正确的商店类型 &7(buy, sell)"),
    dynamic__operation__invalid("&e请输入正确的操作类型 &7(set, plus, minus)"),
    dynamic__buy__not_found("&e找不到收购商品&b %item%"),
    dynamic__buy__player_required("&e收购商品&b %item% &e要求每个玩家的动态值独立，但你未输入玩家参数"),
    dynamic__buy__player_not_required("&e收购商品&b %item% &e不要求每个玩家的动态值独立，但你输入了玩家参数"),
    dynamic__buy__set__global("&a已设置收购商品&e %item% &a的动态值为&e %dynamic%"),
    dynamic__buy__set__player("&a已设置玩家&e %player% &a的收购商品&e %item% &a的动态值为&e %dynamic%"),
    dynamic__buy__plus__global("&a已为收购商品&e %item% &a的动态值增加&e %value%&a，增加后为&e %dynamic%"),
    dynamic__buy__plus__player("&a已为玩家&e %player% &a的收购商品&e %item% &a的动态值增加&e &a%value%，增加后为&e %dynamic%"),
    dynamic__buy__minus__global("&a已为收购商品&e %item% &a的动态值减少&e %value%&a，减少后为&e %dynamic%"),
    dynamic__buy__minus__player("&a已为玩家&e %player% &a的收购商品&e %item% &a的动态值减少&e &a%value%，减少后为&e %dynamic%"),
    dynamic__sell__not_found("&e找不到出售商品&b %item%"),
    dynamic__sell__player_required("&e出售商品&b %item% &e要求每个玩家的动态值独立，但你未输入玩家参数"),
    dynamic__sell__player_not_required("&e出售商品&b %item% &e不要求每个玩家的动态值独立，但你输入了玩家参数"),
    dynamic__sell__set__global("&a已设置出售商品&e %item% &a的动态值为&e %dynamic%"),
    dynamic__sell__set__player("&a已设置玩家&e %player% &a的出售商品&e %item% &a的动态值为&e %dynamic%"),
    dynamic__sell__plus__global("&a已为出售商品&e %item% &a的动态值增加&e %value%&a，增加后为&e %dynamic%"),
    dynamic__sell__plus__player("&a已为玩家&e %player% &a的出售商品&e %item% &a的动态值增加&e &a%value%，增加后为&e %dynamic%"),
    dynamic__sell__minus__global("&a已为出售商品&e %item% &a的动态值减少&e %value%&a，减少后为&e %dynamic%"),
    dynamic__sell__minus__player("&a已为玩家&e %player% &a的出售商品&e %item% &a的动态值减少&e &a%value%，减少后为&e %dynamic%"),
